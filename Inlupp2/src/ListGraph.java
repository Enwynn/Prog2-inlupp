...
public class ListGraph<T> implements Graph<T>, Serializable {
    ...
    private Map<T, LinkedList<Edge<T>>> edges = new Hashtable<>();
    ...
    public void add(T node) {
        ...
        ((Node) node).setId(((Node) node).getName());
    ...
    public boolean alreadyConnected (T node1, T node2) {
    ...
    public Set<T> getNodes() {
        ...
        for (Map.Entry<T, LinkedList<Edge<T>>> n : edges.entrySet()) {
            nodes.add((Node) n.getKey());
        }
        return (Set<T>) nodes;
    }
    ...
    public Map<T, LinkedList<Edge<T>>> getEdges() {
    ...
    private void throwIfNodeNotInGraph (Object node){
        if (!edges.containsKey(node))
            throw new NoSuchElementException("com.company.Node is not in the graph");
    }
